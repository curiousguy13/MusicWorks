...
def IndexSongs(ARTIST,ALBUM,directory,NotDone):
...
def Folders(ARTIST,ALBUM,directory,NotDone):
    try:
        os.mkdir("YourMusic")
    except:
        print "AlreadExists"
    ...
    Copy(ARTIST,ALBUM,directory,NotDone)
    
        
def Copy(ARTIST,ALBUM,directory,NotDone):
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".mp3"):
               ...
               if ALBUM[album_temp] > 5:
                    ...
                    try:
                        shutil.copy(root+'/'+file,"YourMusic/"+artist_temp+"/")
                    except:
                        NotDone.append(root+'/'+file)
                        continue
...
def MyFun():
    ...
    IndexSongs(ARTIST,ALBUM,directory,NotDone)
    Folders(ARTIST,ALBUM,directory,NotDone)
    print "Youza!"
